 ...
 */
export async function searchCitation(url) {
    ...
    if (!response.ok) {
        throw new Error(`MyBib API returned HTTP ${response.status}: ${response.statusText}`);
    }
    const data = (await response.json());
    if (data.status !== "ok") {
        throw new Error(`MyBib API returned unexpected status: "${data.status}"`);
    }
    return data.results ?? [];
}
//# sourceMappingURL=mybib-client.js.map